...
The helper creates `screens/` and `state/`, serves the newest `.html` file in `screens/` at `/`, writes `state/display-info.json`, and exposes `/version` so a default (annotate-off) browser can poll for screen changes. Isolated web starts pass `--annotate`: the printed `url` remains the token-free origin, and `authorize_url` is the private first-visit link. That link validates the session token and issues an HttpOnly, SameSite=Strict cookie; a helper-owned page then navigates to the token-free origin before any authored screen script runs. Only authenticated HTML navigations receive the overlay. Public visits and script fetches get raw screens, without a cookie or overlay, and disk screens stay agent-clean. Wait, annotation, events, flush, and session-end routes remain credential-gated. A prototype may use `?token=` or other query state of its own. Every other path is read from `screens/` at that same path — `/img/blot.webp` serves `screens/img/blot.webp` — so a screen keeps whatever asset layout it was copied from, nesting included. Put the assets the screen references under `screens/` at the paths it asks for, or inline them as data URIs. Anything resolving outside `screens/` is refused.
...
Pick the browser that will load the prototype — one surface. For an annotated preview, use the returned `authorize_url`, not the public `url`; the browser must establish its session cookie before it can annotate. A plain preview still uses `url`. Use the selected URL as returned when that browser is on this machine. When it is not, start with `--host 0.0.0.0` and hand the explorer the helper's returned URL with only the host rewritten to one they can reach. Rewrite only the host; preserve the authorization path and token. Do not also hand localhost. The authorization page replaces its own URL with the token-free origin. Wait talks the bind address with the file token.

Treat `authorize_url` as a bearer credential: deliver it privately to the intended explorer or browser, not to public comments, committed capsules, or shared logs. Do not print the token separately. Anyone holding that link can control the annotation session until it ends. Binding every interface exposes screen files to anything that can reach the port; control-route authentication does not make the screens confidential or encrypt HTTP traffic. Do it only on a network the user trusts, and say so when you hand over the URL.
